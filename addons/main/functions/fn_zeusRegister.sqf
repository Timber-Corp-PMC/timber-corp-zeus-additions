...
private _wait = [player] spawn
{
    ...
    private _hasSSS = isClass (configFile >> "CfgPatches" >> "SSS_main");
    if (_hasSSS) then {
        _moduleList pushBack ["Call extraction", {_this call timberZA_main_fnc_callExtraction}, "\a3\modules_f\data\portraitmodule_ca.paa"];
    };
